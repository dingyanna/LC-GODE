...
Data1 = namedtuple('Data', ['ts_', 'ts_ext_', 'ts_vis_', 'ts', 'ts_ext', 'ts_vis', 'train_ys', 'train_ys_obs', 'train_graph_lis', 'test_ys', 'test_ys_obs', 'test_graph_lis', 'num_train_batch', 'num_test_batch', 'min_', 'max_'])
...
class LatentMultiODE(torchsde.SDEIto):

    def __init__(self, dim_in=2, dim_hid=8, theta=1.0, mu=0.1, sigma=0.5, node_feat=2):
        super(LatentMultiODE, self).__init__(noise_type="diagonal")
        ...
        # )
        self.net = nn.Sequential(
            nn.Linear(dim_hid+dim_hid , dim_hid*4),
            nn.GELU(),#tanh
        ...
        self.seq_encoder = SelfAttentionNetwork(dim_in, dim_hid*2)
        #self.seq_encoder = AutoRegressiveGRU(dim_in, dim_hid, dim_hid*2)
        #self.seq_encoder = Encoder_GRU(dim_in, dim_hid, dim_hid*2)
        self.graph_encoder = GraphEncoder(node_feat, dim_hid, args)
    ...
    def f(self, t, y):  # Approximate posterior drift.
        ...
        res = self.net(torch.cat(( y,self.graph_emb), dim=1))
        #print(res.shape, '\n')
        ################## !!!!!!!!!! the result of net should have the same shape as y!!!!!!!
        return  res
        
     
    def forward(self, observed, ts, graph, batch_size, eps=None):
          
        self.qy0_mean, self.qy0_logvar = self.seq_encoder(observed)
        self.graph_emb = self.graph_encoder(graph)
        #self.graph_emb = self.graph_emb.repeat(1,batch_size,1).reshape(-1,self.qy0_mean.shape[-1])
        self.graph_emb = self.graph_emb.repeat_interleave(batch_size,dim=0) 
    ...
    def sample_q(self, observed, ts, graph, batch_size, eps=None, bm=None):
        # observed: 
   
        self.qy0_mean, self.qy0_logvar = self.seq_encoder(observed)
        self.graph_emb = self.graph_encoder(graph) 
        self.graph_emb = self.graph_emb.repeat(batch_size,1,1).reshape(-1,self.qy0_mean.shape[-1])
...
class LatentMultiSDE(torchsde.SDEIto):

    def __init__(self, dim_in=2, dim_hid=8, theta=1.0, mu=0.1, sigma=0.5, node_feat=2):
        super(LatentMultiSDE, self).__init__(noise_type="diagonal")
        logvar = math.log(sigma ** 2 / (2. * theta))
...
def get_adj_cnn( arch_str):   
    nodes = arch_str.split('+') # break into nodes
    X = torch.zeros((4,2)) # avg in_deg, out_deg
    adj = np.zeros((4,4)) 
    for i in range(1,len(nodes)+1): 
        upstream = nodes[i-1].split('|')[1:-1] # get upstream nodes and edges
     
        for node in upstream:
            op, source = node.split('~')
            if weight_dict[op] == 0:
                # no connection
                continue 
            adj[int(source),i] = weight_dict[op] 
            X[int(source),1] += 1.
            X[i,0] += 1.
    
    # print('arch_str', arch_str) 
    # print(X)    
    #supergraph_edge_index = torch.LongTensor(supergraph_edge_index) 
    A = adj + np.eye(adj.shape[0]) * .5  
    D = np.diag(1 / np.sqrt(A.sum(1)))
    A = D @ A @ D 
    edge_index = torch.LongTensor(np.stack(np.where(A!=0))) 
    edge_attr = torch.FloatTensor(A[A!=0]).reshape(-1,1)  
     
    X = X / CONSTANT
    #print('Nodal feature size', X.shape)
    topo = torch.tensor([X.shape[0], edge_index.shape[0]], dtype=torch.int).reshape(1,-1)
    enc_graph = Data(x=X, edge_index=edge_index, edge_attr=edge_attr, topo=topo )
      
    return enc_graph  #, supergraph_edge_index, num_weight, num_node
...
def make_cnn_lc():
    ...
    graph_lis = []
    fn = f"/home/dingy6/nn_project/LC-ODE/cnn/arch_str_{args.source_data}.txt"
    with open(fn, 'r') as file:
        lines = [line.strip() for line in file]

    for i in range(lc.shape[0]): 
        graph_lis.append( get_adj_cnn( lines[i]) )

    #idx = np.random.permutation(lc.shape[0])
    idx = np.arange(lc.shape[0])
    lc = lc[ idx ]
    graph_lis = [graph_lis[i] for i in idx]
    ############## Normalization 
    #lc = lc[idx ]
    #lc = np.load("car") 
    #lc = normalize(lc )  
    ...
    train_graph_lis = [graph_lis[i] for i in train_id]
    test_graph_lis = [graph_lis[i] for i in test_id]
    ...
    train_graph_lis = DataLoader(train_graph_lis, batch_size=args.train_batch_size,shuffle=False, num_workers=3) 
    test_graph_lis =  DataLoader(test_graph_lis,  batch_size=args.train_batch_size,shuffle=False, num_workers=3) 

    num_train_batch = len(train_ys_obs)
    num_test_batch = len(test_ys_obs )
    train_ys_obs = inf_generator(train_ys_obs)
    train_graph_lis   = inf_generator(train_graph_lis)
    train_ys = inf_generator(train_ys)
    test_ys_obs = inf_generator(test_ys_obs)
    test_graph_lis = inf_generator(test_graph_lis)
    test_ys  = inf_generator(test_ys) 

 
    return Data1(ts_, ts_ext_, ts_vis_, ts, ts_ext, ts_vis, train_ys, train_ys_obs, train_graph_lis, test_ys, test_ys_obs, test_graph_lis, num_train_batch, num_test_batch, min_, max_ )

def get_node_id( dim_lis):
    '''
    Construct a map: layerid_order -> node id
    The 
    ''' 
    node_id_map = {}
    count = 0
    for i in range(len(dim_lis)):
        for j in range(dim_lis[i]):
            node_id_map[f"{i}_{j}"] = count 
            count += 1
        if i < len(dim_lis) - 1:
            node_id_map[f"{i}_bias"] = count 
            count += 1 
    return node_id_map  

def get_adj_mlp(dim_lis):   
    num_node = 0
    num_weight = 0
    node_layer = []
    for i in range(len(dim_lis)-1):
        num_node += dim_lis[i] + 1 
        node_layer += ([i] * dim_lis[i] )   
        node_layer.append(i) 
        num_weight += dim_lis[i+1]*dim_lis[i] + dim_lis[i]
    num_node += dim_lis[-1]    
    node_layer += ([len(dim_lis)-1] * dim_lis[-1])
      
    adj = np.zeros((num_node,num_node))
    ##np.fill_diagonal(adj, np.ones(num_node))
    node_id_map = get_node_id(dim_lis) 
    assert(num_node == len(node_id_map)) 
    
    edge_index = [] 
    edge_layer = []   
      
    for i in range(len(dim_lis) - 1):
        for j in range(dim_lis[i+1]):
            for k in range(dim_lis[i]):
                sender = node_id_map[f"{i}_{k}"]
                receiver = node_id_map[f"{i+1}_{j}"]
                adj[sender, receiver] = 1 
                #edge_index += [[sender,receiver] ] 
                edge_layer.append(i)
        for j in range(dim_lis[i+1]):
            sender = node_id_map[f"{i}_bias"]
            receiver = node_id_map[f"{i+1}_{j}"]
            adj[sender, receiver] = 1  
            #edge_index += [[sender,receiver] ]
            edge_layer.append(i)  

    #edge_index = np.stack(edge_index).transpose()
    ...
    A = D @ A @ D
    edge_attr = torch.FloatTensor(A[A!=0]).reshape(-1,1)
    in_deg = torch.FloatTensor(adj.sum(1))
    out_deg = torch.FloatTensor(adj.sum(0))
    X = torch.stack((in_deg/adj.sum(),out_deg/adj.sum())).T
    #print('Nodal feature size', X.shape)
    topo = torch.tensor([X.shape[0], edge_index.shape[0]], dtype=torch.int).reshape(1,-1)
    enc_graph = Data(x=X, edge_index=edge_index, edge_attr=edge_attr, topo=topo )
    return enc_graph  #, supergraph_edge_index, num_weight, num_node

def get_adj_mlp_simple(dim_lis):   
    num_node = len(dim_lis) 
    adj = np.zeros((num_node,num_node))
    ##np.fill_diagonal(adj, np.ones(num_node)) 
    ...
    X = np.asarray(dim_lis, dtype=float)/ max(dim_lis)
    X = X.reshape(-1,1)
...
def make_mlp_lc():
    ...
    graph_lis = [] 
    #lc = [] # old data
    if args.source_data == 'kr-vs-kp':
        ids = np.asarray(range(len(lc))) 
        ds = f"{args.source_data}_1"
        #ids = range(50) # old data
    else:
        ids = np.loadtxt(f"/home/dingy6/nn_project/LC-ODE/mlp/{args.source_data}_id.txt")
        ds = args.source_data
        #print('Using the oldest version of data') # old data
        #ds = f"{args.source_data}_1" # old data
        #ids = range(50) # old data
    #idx = np.random.permutation(lc.shape[0])
    idx = np.arange(lc.shape[0])
    lc = lc[idx]
    ...
    for i in ids: 
        with open(f'/home/dingy6/nn_project/LC-ODE/mlp/{ds}/config{int(i)}.json','r') as f:
            
            dims = json.load(f)['dims'] 
        if args.mlp_graph == 'simple':
            graph_lis.append( get_adj_mlp_simple( dims ) )
        else:
            graph_lis.append( get_adj_mlp( dims ) )
        #lc.append( (np.load(f'/home/dingy6/nn_project/LC-ODE/mlp/{ds}/val_loss{i}.npy')) ) # old data
    #lc = np.stack(lc ) # old data
    ############## Normalization 
    #lc = lc[idx ]
    #lc = np.load("car") 
    #lc = normalize(lc )  
...
def test(global_step, model):
    ts_, ts_ext_, ts_vis_, ts, ts_ext, ts_vis, train_ys, train_ys_obs, train_graph_lis, test_ys, test_ys_obs, test_graph_lis, num_train_batch, num_test_batch, min_, max_ = make_data()
    ...
    with torch.no_grad():
        ...
        for ex in range(num_train_batch):
            ...
            graph = get_next_batch_new(train_graph_lis).to(device)
         
            z0, z0_std = model.seq_encoder(ys_obs) 
...
def main():
    def plot(): 
        with torch.no_grad():
            for ex in range(num_test_batch):
                ...
                graph = get_next_batch_new(test_graph_lis).to(device)
                                            # previous time list: ts_vis_ 
                zs = model.sample_q(ys_obs, ts=ts, graph=graph, batch_size=vis_batch_size, eps=eps ).squeeze()
                # [T, N_sample, vis_batch_size]
                ...
                for i in range( min(20,ys.shape[0])):
    ...
    if args.diff_eq == 'sde':
        args.method = 'euler'
        model = LatentMultiSDE(mu=args.mean, sigma=args.std, dim_hid=args.latent_dim, node_feat=args.node_feat).to(device)
    else:
        args.method = 'rk4'
        model = LatentMultiODE(mu=args.mean, sigma=args.std, dim_hid=args.latent_dim, node_feat=args.node_feat).to(device)
    init_weights(model)
    ...
    for global_step in tqdm.tqdm(range(args.train_iters)):
        ...
        for ex in range(num_train_batch):
            ...
            zs, kl = model(ys_obs, ts=ts_ext, graph=graph, batch_size=args.batch_size)
            ...
            #print(ranking_loss)
            loss = 5000*err_ic -logpy + kl * kl_scheduler.val
        ...
        # elif global_step == args.train_iters - 1:
        #     test(global_step, model)  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_pooling', type=str, default='diff')
    parser.add_argument('--mlp_graph', type=str, default='full')
    parser.add_argument('--curve_type', type=str, default='loss')
     
    args = parser.parse_args()

    if args.data == 'cnn':
        args.train_dir =  f"./wgraph_res/{args.source_data}_{args.curve_type}_graph_{args.diff_eq}_seed{args.seed}/{today}"
    else:
        args.train_dir =  f"./wgraph_res/{args.source_data}_{args.curve_type}_graph_{args.mlp_graph}_{args.diff_eq}_seed{args.seed}/{today}"
